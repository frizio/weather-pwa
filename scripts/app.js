...
(function() {
  ...
  // Using Cache first, then Network STRATEGY
  app.getForecast = function(key, label) {
    var url = weatherAPIUrlBase + key + '.json';
    // Try and get data from the CACHE 
    if ('caches' in window) {
      caches.match(url).then(function(response) {
        if (response) {
          response.json().then(function(json) {
            // Onlu update if the XHR is still pending, 
            // otherwise the XHR  has already returned and provided the last data
    ...
    // Type and make an AJAX request (over the NETWORK)
